<template>
  <div class="list">
    <el-table :data="list" style="width: 100%" height="70vh">
      <el-table-column prop="name" label="标题" width="400">
        <template slot-scope="scope">
          <p class="fd1" @click="updata">{{ scope.row.name }}</p>
        </template>
      </el-table-column>
      <el-table-column prop="count" label="阅读量" width="200"></el-table-column>
      <el-table-column prop="publisher" label="发布人" width="200"></el-table-column>
      <el-table-column prop="state" label="状态" width="200"></el-table-column>
      <el-table-column prop="date" label="发布时间"></el-table-column>
    </el-table>
  </div>
</template>

<script>
// import { mapActions } from 'vuex';
export default {
  data() {
    return {};
  },
  methods: {
    updata() {
      this.$router.push('./content/forthismoment');
    },
  },
  props: {
    list: null,
  },
  watch: {
    data: function(val) {
      console.log(val);
    },
  },
};
</script>

<style load="loss" scoped>
@import '~@/assets/style.less';
</style>
